...
class yara_tools(object):
	...
	def __init__(self, rule_name):
		...
		self.__string_struct = {'type': '', 'identifier': '',
							  'strings': '', 'modifiers': '', 'condition': '', 'comment': ''}
	...
	def create_condition_group(self,name,default_boolean='and'):
		...
		group_struct={'default_boolean' : 'and',
								'conditions' : []
								}
	...
	def add_strings(self, strings, modifiers=False, identifier=False, condition=False, condition_group='default', string_type=False, comment=False):
	...
	def add_binary_strings(self, data, size_limit=False, modifiers=False, identifier=False, condition=False, condition_group=False, comment=False):
		...
		if type(data) == list:
			for i in range(len(data)):
				if size_limit:
					data[i] = binary_template % (self.raw_to_hex(data[i][0:int(size_limit)]))
				else:
					data[i] = binary_template % (self.raw_to_hex(data))
		else:
			if size_limit:
		...
		self.add_strings(strings=data, modifiers=modifiers, identifier=identifier, condition=condition, string_type='binary', comment=comment)
	...
	def ret_complete_rule(self, rule_name, condition, meta=False, strings=False, imports=False, includes=False):
	...
	def build_rule(self):
		...
		if tmp_condition or tmp_strings:
			rule = self.ret_complete_rule(rule_name=self.__name, 
